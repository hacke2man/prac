#include <ncurses.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#include "parse.h"
#include "util.h"
#include "type.h"

void colorch(char ch, int color){
  attron(COLOR_PAIR(color));
  addch(ch);
  attroff(COLOR_PAIR(color));
}

void dispSting(char * str, char * typed, unsigned int buffer)
{
  int attrib = 3;

  move(LINES/2, 0);

  for(int i = 0; i < buffer; i++)
    addch(' ');

  for(int i = 0; i < (COLS - buffer) && i < strlen(str) - 1; i++)
  {
    if(i > COLS/2 - buffer - 1 || strlen(typed) == 0){
      attrib = 3;
    }else{
      if(str[i] == typed[i])
      {
        attrib = 1;
      }
      else
        attrib = 2;
    }

    colorch(str[i], attrib);

  }
  addch(' ');
}

void Type()
{
  start_color();
  raw();
  noecho();
  refresh();
  curs_set(0);
  keypad(stdscr, TRUE);
  int wrongfg = COLOR_RED;
  int correctfg = COLOR_GREEN;
  int nextfg = COLOR_WHITE;
  int bg = COLOR_BLACK;
  init_pair(1, correctfg, bg);
  init_pair(2, wrongfg, bg);
  init_pair(3, nextfg, bg);

  char * test = malloc(sizeof(char) * 1000);
  test[1000 - 1] = '\0';
  srand(1);
  for(int i = 0; i < 999; i++)
    test[i] = rand()%(122 - 97) + 97;

  char * typed = malloc(sizeof(char) * strlen(test) + 1);
  typed[0] = '\0';
  mvprintw(LINES/2 + 1, COLS/2, "^");
  dispSting(test, typed, COLS/2);
  refresh();

  bool newmax = FALSE;
  float acc = 1.0;
  int correct = 0;
  int ch;
  int i = 0;
  int max = 0;
  char tmp[2];
  tmp[1] = '\0';
  while(ch != 27){
    ch = getch();
    i++;
    if(i >= max)
    {
      max = i;
      newmax = TRUE;
    }
    switch (ch)
    {
      case 26:
        break;
      case KEY_BACKSPACE:
        i--;
        typed[i] = '\0';
        i--;
        typed[i] = '\0';

        if(COLS/2 - i < 0)
        {
          dispSting(&test[i - COLS/2], &typed[i - COLS/2], 0);
        }else{
          dispSting(test, typed, COLS/2 - i);
        }

        break;
      default:
        tmp[0] = ch;
        strcat(typed, tmp);
        if(COLS/2 - i < 0)
        {
          dispSting(&test[i - COLS/2], &typed[i - COLS/2], 0);
        }else{
          dispSting(test, typed, COLS/2 - i);
        }

        if(newmax)
          correct = typed[i-1] == test[i-1] ? correct + 1 : correct;

        if(newmax)
          acc = 100*(float)correct/max;
        mvprintw(LINES/2 + 2, COLS/2, "%%%d ", (int)roundf(acc));

        refresh();
        break;
    }
    newmax = FALSE;
  }

  clear();
  refresh();
  curs_set(1);
}
